...
# # #                                   TO RUN THE CODE:
# # #
# # # a = LandscapeFold(['GCGCAAAAGCGC'],storeGraphs=True) #define the object with the parameters you want
# # # sol = a.mainLandscapeCalculation() #calculate the FE Lanscape
...
class LandscapeFold:
    ...
    def __init__(self, sequences, DNA = [False,False], sequenceConcentrations = [1,1], b = 0.8/0.33, T = 310.15, 
                  ...
                  toSave = True, saveSmall = False, tryToLoadVariables = True, maxSizeSTable = 10**4, 
                  ...
                  considerC3andC4 = True,  corruptFESeed = 0, unboundButCouldBindPenalties = [0,0],
                  allowedBPs=None):
        
        self.version = '8_20' #don't forget to change this when we update the code
...
# sequenceConcentrations has no effect outside of kinetics (it is a placeholder; in the future we should 
#     be able to give more precise entropic costs of binding based on the concentrations of each sequence)
...
# allowPseudoknots defines whether to allow intramolecular pseudoknots in the computation.
...
#     If two sequences are given, then frozenBPs is assumed to refer to the bases BEFORE
#     the sequences have been sorted, but AFTER the linker has been added. For example,
#     sequences = ['GGGAAACCC', 'AAACCCUUU']; frozenBPs = [[0, 8], [1, 7], [16, 22], [17, 21]]
#     means the first and second bp of the 'GG...' sequence are bound to the last and second-to-last,
#     while the second and third bp of the 'AA...' sequence are bound to the second- and third-to-last.
        
# toSave is a boolean which determines whether to save the results

# saveSmall is a boolean. If it is False (default) it saves the class as is after doing the 
#     landscape calculation. If it is True, it forgoes making it easy to later change 
#     the energy and entropy of base pairs, and instead saves the landscape for a given 
#     energy and entropy (etc.). If numStructures > 1e6, saveSmall is automatically set to True.
...
# allowedBPs is a 2D np array of size numNt by numNt describing which bases are 
#     allowed to bond to which others. If it is given, it takes the place of matrix B 
#     in createSTable(). This allows us, for example, to start from a given structure
#     and find all the structures created by opening base pairs in that structure.
#     NOTE: FOR NOW, SAVING/LOADING DOES NOT TAKE INTO ACCOUNT WHAT allowedBPS WAS SET TO
#     If two sequences are given, then allowedBPs is assumed to refer to the bases BEFORE
#     the sequences have been sorted, but AFTER the linker has been added, just like frozenBPs. 
# =============================================================================
...
# =============================================================================                
        sequences, DNA, sequenceConcentrations, frozenBPs, allowedBPs = self.basicCorrectionsToInput(
                sequences, DNA, sequenceConcentrations, frozenBPs, allowedBPs, minNtsInHairpin) 
#            perform basic corrections to input like sorting sequences, making sure they're in a list, etc.
        
        self.sequences = sequences            
        self.DNA = DNA
        self.sequenceConcentrations = sequenceConcentrations
...
# =============================================================================
        self.sequence,self.sequenceInNumbers,self.numSequences,self.numNt,\
            self.numNt1,self.numNt2,self.linkerPos = self.makeSequenceInNumbers()
...
# =============================================================================
        if self.toSave or self.tryToLoadVariables:
            ...
            self.fileVars, self.fileTxt, self.folderName, self.fileFigs = [None, None, None, None]
            

    def basicCorrectionsToInput(self, sequences, DNA, sequenceConcentrations, 
                                frozenBPs, allowedBPs, minNtsInHairpin):
# =============================================================================
# perform basic corrections to input like sorting sequences, making sure they're in a list, etc.
# =============================================================================
...
#       Make sure sequences is sorted so that we load the results if we tried it with opposite order
        if not sequences == sorted(sequences):            
            lenLinker = 2*minNtsInHairpin  # length of linker that will be established
            if frozenBPs:
                # Treat the frozenBPs assuming they were created with knowledge of the linker,
                # but without knowing the sequences were going to be swapped.
                frozenBPsInFirstSequence = np.array(
                        [[1 if i < len(sequences[0]) else 0 for i in j] 
                        for j in frozenBPs])
                frozenBPsChangeAfterSeqSwap = (
                        lenLinker + len(sequences[1])) * frozenBPsInFirstSequence + (
                                lenLinker + len(sequences[0])) * (frozenBPsInFirstSequence - 1)
                frozenBPs_afterSeqSwap = np.array(frozenBPs) + frozenBPsChangeAfterSeqSwap
                frozenBPs = sorted(frozenBPs_afterSeqSwap.tolist())
                
            if allowedBPs is not None:
                # We need to rearrange the columns of allowedBPs to account for the sequence swap
                allowedBPs_swappedCols = np.zeros(allowedBPs.shape)
                allowedBPs_swappedCols[:, range(len(sequences[1]))] = allowedBPs[
                        :, range(len(sequences[0]) + lenLinker, len(allowedBPs))]
                allowedBPs_swappedCols[:, range(
                        len(sequences[1]) + lenLinker, len(allowedBPs))] = allowedBPs[
                        :, range(len(sequences[0]))]
                
                allowedBPs_swappedColsRows = np.zeros(allowedBPs.shape)
                allowedBPs_swappedColsRows[range(len(sequences[1])), :] = allowedBPs_swappedCols[
                        range(len(sequences[0]) + lenLinker, len(allowedBPs)), :]
                allowedBPs_swappedColsRows[range(
                        len(sequences[1]) + lenLinker, len(allowedBPs)), :] = allowedBPs_swappedCols[
                        range(len(sequences[0])), :]
                allowedBPs = allowedBPs_swappedColsRows
                
            DNA = [DNA[1], DNA[0]]
            sequences = [sequences[1], sequences[0]]
            sequenceConcentrations = [sequenceConcentrations[1], sequenceConcentrations[0]]

                
#        If we have DNA with U's and no T's, change it to RNA, and vice versa
#            If we have a mix of U's and T's replace them based on whether it's DNA or RNA
        for i in range(len(sequences)):
            sequences[i] = sequences[i].upper()
            if DNA[i]:
                if sequences[i].count('U') and not sequences[i].count('T'):
                    DNA[i] = False
                    print('FOUND U''s and no T''s in DNA sequence. Changing to RNA')
                elif sequences[i] != sequences[i].replace('U','T'):
                    sequences[i] = sequences[i].replace('U','T')
                    print('FOUND U''s in DNA sequence along with T''s. Changing U''s to T''s')
            else:
                if sequences[i].count('T') and not sequences[i].count('U'):
                    DNA[i] = True
                    print('FOUND T''s and no U''s in RNA sequence. Changing to DNA')
                elif sequences[i] != sequences[i].replace('T','U'):
                    sequences[i] = sequences[i].replace('T','U')
                    print('FOUND T''s in RNA sequences along with U''s. Changing T''s to U''s')
            
        return(sequences, DNA, sequenceConcentrations, frozenBPs, allowedBPs)
        

    def makeSequenceInNumbers(self):
    ...
    def mainLandscapeCalculation(self):
# =============================================================================
...
# =============================================================================
        ...
        if self.tryToLoadVariables:
            ...
            if loadedVars:
                ...
                myPrintFxn('numStems = ' + str(loadedVars.numStems), loadedVars.fileTxt)
                myPrintFxn('numStructures = ' + str(loadedVars.numStructures), loadedVars.fileTxt)
...
# =============================================================================   
        ...
        # =============================================================================
        # given a list of base pairs that we want fixed, what stems do they correspond to?
        # frozenStems is a list, where frozenStems[i] is itself a list of
        # stems containing all the base pairs of the i'th stem in frozenBPs. Thus,
        # in order to keep all of frozenBPs fixed, each possible structure must have
        # one stem from each list in each element of frozenStems.
        # =============================================================================
...
#           linkedStems[i] == True if stem i corresponds to the two strands being bound, and False otherwise
        ...
        if self.considerC3andC4:
            C3 = self.makeThreewayCompatibilityTensor() #don't store it in self because it's huge
            C4 = self.makeFourwayCompatibilityTensor(C3) #don't store it in self because it's huge
        else:
            C3 = True; C4 = True #so the variables exist
...
#            self.stemEnergies, self.stemEntropies = self.calculateStemFreeEnergiesSingle()
        ...
        self.STARTtime = time.time() - timeAtStart
        if self.printProgressUpdate:
            myPrintFxn('Time for START function = ' + str(self.STARTtime), self.fileTxt)
...
# =============================================================================
        ...
        self.numStructures, self.structures, self.linkedStructures = self.enumerateStructures(C3, C4)
        
        self.PERMUtime = time.time() - permuFxnStartTime
        myPrintFxn('numStructures = ' + str(self.numStructures), self.fileTxt)
        if self.printProgressUpdate:
            myPrintFxn('Time for PERMU function = ' + str(self.PERMUtime), self.fileTxt)
        ...
        self.CHECKtime = time.time() - checkFxnStartTime
        if self.printProgressUpdate:
            myPrintFxn('Time for CHECK function = ' + str(self.CHECKtime), self.fileTxt)

        return(self.postCalculationFxn())
    
    
    def twoStrandLandscapeCalculation(self):
# =============================================================================
#   Alternative to mainLandscapeCalculation() if we have two sequences
#   If we have two strands, it's faster to consider each strand separately, 
#   then  enumerate the possible combinations of those, and add on the heterodimer
#   and if desired the homodimers.
# =============================================================================
        
        if self.storeGraphs:  # then it's unclear how it is best to return the graphs. 
            # We don't allow storeGraphs in this function for now. 
            print("ERROR: can't store graphs with twoStrandLandscapeCalculation()")
            return(None)
        
        self.timeAtStart = time.time()
        
        cumTotNumStemsVec, cumTotNumStructuresVec, fullSTableBPs, fullStructuresList,\
            allComponentGraphs, allNumVs, bondFECounts, dangling5Count, dangling3Count, \
            unboundButCouldBindCounts = self.monomersAndDimer()
# =============================================================================
#   We've been treating the three species separately so far. 
#   Now combine the individual monomer structures to make the final list of structures.
# =============================================================================        
        self.structures = ([fullStructuresList[i] + fullStructuresList[j] 
                            for i in range(cumTotNumStructuresVec[0]) 
                            for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                            fullStructuresList[cumTotNumStructuresVec[1]:])
        
        self.numStructures = len(self.structures)
        self.STableBPs = fullSTableBPs
        self.numStems = len(self.STableBPs)
        self.STableStructure = [[x[0] for x in i] + [x[1] for x in i] for i in self.STableBPs] 
        self.linkedStructures = np.array([False] * (cumTotNumStructuresVec[0] * (cumTotNumStructuresVec[1] - cumTotNumStructuresVec[0])) + 
                                         [True] * (cumTotNumStructuresVec[2] - cumTotNumStructuresVec[1]), dtype=bool)
        
        self.allComponentGraphs = ([{key: value + allComponentGraphs[i][key] for key, value in allComponentGraphs[j].items()}
                                    for i in range(cumTotNumStructuresVec[0]) 
                                    for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                                    allComponentGraphs[cumTotNumStructuresVec[1]:])
        
        self.allNumVs = np.array([allNumVs[i] + allNumVs[j] 
                                  for i in range(cumTotNumStructuresVec[0]) 
                                  for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                                  allNumVs[cumTotNumStructuresVec[1]:])
        
        #lil_matrix + lil_matrix is slow, so we convert to a different sparse type
        bondFECounts = [[i.tocsr() for i in j] for j in bondFECounts]
        dangling5Count = [[i.tocsr() for i in j] for j in dangling5Count]
        dangling3Count = [[i.tocsr() for i in j] for j in dangling3Count]
        
        #Summing these matrices is by far the slowest part of the code right now.
        #It takes approx 10x more time than the preceding landscape calculations.
        #This is generally still much faster than the previous method of enumeration though.
        self.bondFECounts = ([[x + y for x,y in zip(bondFECounts[i], bondFECounts[j])]
                              for i in range(cumTotNumStructuresVec[0]) 
                              for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                              bondFECounts[cumTotNumStructuresVec[1]:])
        self.dangling5Count = ([[x + y for x,y in zip(dangling5Count[i], dangling5Count[j])]
                              for i in range(cumTotNumStructuresVec[0]) 
                              for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                              dangling5Count[cumTotNumStructuresVec[1]:])
        self.dangling3Count = ([[x + y for x,y in zip(dangling3Count[i], dangling3Count[j])]
                              for i in range(cumTotNumStructuresVec[0]) 
                              for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                              dangling3Count[cumTotNumStructuresVec[1]:])
        self.unboundButCouldBindCounts = np.array([unboundButCouldBindCounts[i] + unboundButCouldBindCounts[j] 
                                  for i in range(cumTotNumStructuresVec[0]) 
                                  for j in range(cumTotNumStructuresVec[0], cumTotNumStructuresVec[1])] +
                                  unboundButCouldBindCounts[cumTotNumStructuresVec[1]:])
    
        return(self.postCalculationFxn())
    
    
    def monomersAndDimer(self):
        
        numSpecies = 3  # We currently don't allow homodimers, but may in the future.
        
        cumTotNumStructures = 0
        cumTotNumStructuresVec = np.zeros(numSpecies, dtype = int)
        cumTotNumStems = 0
        cumTotNumStemsVec = np.zeros(numSpecies, dtype=int)
        
    # =============================================================================
    #     first add single sequence #1
    # =============================================================================
        onlyConsiderBondedStrands = False
        sequences = [self.sequences[0]] #no need to copy
        lenSeq1 = len(self.sequences[0])
        if self.frozenBPs:
            frozenBPs = [i for i in self.frozenBPs if i[0] < lenSeq1 and i[1] < lenSeq1]
        else:
            frozenBPs = self.frozenBPs
            
        if self.allowedBPs is not None:
            allowedBPs = self.allowedBPs[:lenSeq1, :lenSeq1]
        else:
            allowedBPs = self.allowedBPs
        
        seq1 = LandscapeFold(sequences, DNA = self.DNA, sequenceConcentrations = [1,1], b = self.b, 
                            T = self.T, vs=self.vs, duplexEntropyPenaltyInKB = 0, minBPInStem = self.minBPInStem, 
                            numParForLoops = self.numParForLoops, storeGraphs = False, makeFigures = False,
                            printProgressUpdate = self.printProgressUpdate, allowPseudoknots = self.allowPseudoknots, 
                            minNumStemsInStructure = self.minNumStemsInStructure, substems = self.substems, 
                            frozenBPs = frozenBPs, toSave = self.toSave, saveSmall = self.saveSmall, 
                            tryToLoadVariables = self.tryToLoadVariables, maxSizeSTable = self.maxSizeSTable, 
                            minNtsInHairpin = self.minNtsInHairpin, 
                            onlyAllowSubsetsOfLongestStems = self.onlyAllowSubsetsOfLongestStems, 
                            onlyConsiderSubstemsFromEdges = self.onlyConsiderSubstemsFromEdges, 
                            onlyConsiderBondedStrands = onlyConsiderBondedStrands, 
                            considerTabulatedLoops = self.considerTabulatedLoops, unmatchedBPPenalty = self.unmatchedBPPenalty, 
                            considerAllAsTerminalMismatches = self.considerAllAsTerminalMismatches,
                            includeTerminalMismatches = self.includeTerminalMismatches, 
                            includeFlushCoaxialStacks = self.includeFlushCoaxialStacks,
                            includeDanglingEnds = self.includeDanglingEnds, 
                            includeTerminalAUATPenalties = self.includeTerminalAUATPenalties, 
                            considerC3andC4 = self.considerC3andC4, corruptFESeed = self.corruptFESeed, 
                            unboundButCouldBindPenalties = self.unboundButCouldBindPenalties,
                            allowedBPs=allowedBPs)
        
        
        seq1 = seq1.mainLandscapeCalculation()
        
        cumTotNumStructuresVec[0] = cumTotNumStructures + seq1.numStructures
        cumTotNumStructures = cumTotNumStructuresVec[0]
        
        cumTotNumStemsVec[0] = cumTotNumStems + seq1.numStems
        cumTotNumStems = cumTotNumStemsVec[0]        
    
    # =============================================================================
    #     next add single sequence #2
    # =============================================================================
        onlyConsiderBondedStrands = False
        sequences = [self.sequences[1]] #no need to copy
        if self.frozenBPs:
            lenLinker = 2 * self.minNtsInHairpin
            frozenBPs = [i for i in self.frozenBPs if i[0] > lenSeq1 and i[1] > lenSeq1]
            frozenBPs = [[i[0] - lenSeq1 - lenLinker, i[1] - lenSeq1 - lenLinker] for i in frozenBPs]
        else:
            frozenBPs = self.frozenBPs

        if self.allowedBPs is not None:
            lenLinker = 2 * self.minNtsInHairpin
            allowedBPs = self.allowedBPs[(lenSeq1 + lenLinker):, (lenSeq1 + lenLinker):]
        else:
            allowedBPs = self.allowedBPs

        seq2 = LandscapeFold(sequences, DNA = self.DNA, sequenceConcentrations = [1,1], b = self.b, 
                            T = self.T, vs=self.vs, duplexEntropyPenaltyInKB = 0, minBPInStem = self.minBPInStem, 
                            numParForLoops = self.numParForLoops, storeGraphs = False, makeFigures = False,
                            printProgressUpdate = self.printProgressUpdate, allowPseudoknots = self.allowPseudoknots, 
                            minNumStemsInStructure = self.minNumStemsInStructure, substems = self.substems, 
                            frozenBPs = frozenBPs, toSave = self.toSave, saveSmall = self.saveSmall, tryToLoadVariables = self.tryToLoadVariables, 
                            maxSizeSTable = self.maxSizeSTable, minNtsInHairpin = self.minNtsInHairpin, 
                            onlyAllowSubsetsOfLongestStems = self.onlyAllowSubsetsOfLongestStems, 
                            onlyConsiderSubstemsFromEdges = self.onlyConsiderSubstemsFromEdges, 
                            onlyConsiderBondedStrands = onlyConsiderBondedStrands, 
                            considerTabulatedLoops = self.considerTabulatedLoops, unmatchedBPPenalty = self.unmatchedBPPenalty, 
        ...
        seq2 = seq2.mainLandscapeCalculation()
        
        cumTotNumStructuresVec[1] = cumTotNumStructures + seq2.numStructures
        cumTotNumStructures = cumTotNumStructuresVec[1]
        
        cumTotNumStemsVec[1] = cumTotNumStems + seq2.numStems
        cumTotNumStems = cumTotNumStemsVec[1]  
        
        
    # =============================================================================
    #     now consider strands 1 and 2 bonded (heterodimer)
    # =============================================================================
            
        onlyConsiderBondedStrands = True
        
        seqs12 = LandscapeFold(self.sequences, DNA = self.DNA, sequenceConcentrations = [1,1], b = self.b, 
                            T = self.T, vs=self.vs, duplexEntropyPenaltyInKB = self.duplexEntropyPenaltyInKB, minBPInStem = self.minBPInStem, 
                            numParForLoops = self.numParForLoops, storeGraphs = False, makeFigures = False,
                            printProgressUpdate = self.printProgressUpdate, allowPseudoknots = self.allowPseudoknots, 
                            minNumStemsInStructure = self.minNumStemsInStructure, substems = self.substems, 
                            frozenBPs = self.frozenBPs, toSave = self.toSave, saveSmall = self.saveSmall, tryToLoadVariables = self.tryToLoadVariables, 
                            maxSizeSTable = self.maxSizeSTable, minNtsInHairpin = self.minNtsInHairpin, 
                            onlyAllowSubsetsOfLongestStems = self.onlyAllowSubsetsOfLongestStems, 
                            onlyConsiderSubstemsFromEdges = self.onlyConsiderSubstemsFromEdges, 
                            onlyConsiderBondedStrands = onlyConsiderBondedStrands, 
        ...
        seqs12 = seqs12.mainLandscapeCalculation()
                            
        cumTotNumStructuresVec[2] = cumTotNumStructures + seqs12.numStructures        
        cumTotNumStemsVec[2] = cumTotNumStems + seqs12.numStems
                
    #    for sequence #2, we need to adjust STable to include the bps taken
    #    up by the first sequence and by the linkers
        
        lenLinker = len(self.sequences[0]) + len(seqs12.linkerPos)
        fullSTableBPs = seq1.STableBPs + [[[i+lenLinker for i in j] for j in k] for k in seq2.STableBPs] + seqs12.STableBPs
        
        #The stems now have a different numbering when we treat the fullSTableBPs as the STable
        fullStructuresList = seq1.structures + [[i + cumTotNumStemsVec[0] for i in j] for j in seq2.structures] + \
                [[i + cumTotNumStemsVec[1] for i in j] for j in seqs12.structures]

    # =============================================================================
    #     Some of the stems returned by STable for the dimer cases do not have the strands bonded
    #     and therefore are exactly equivalent to previous stems. So now we
    #     need to redefine those stems (and simultaneously correct fullPossiblePermutationsList) 
    #     so that we have a full list of only one copy of each region
    # =============================================================================
        stems3 = range(cumTotNumStemsVec[1], cumTotNumStemsVec[2]) 
        duplicateStems = [i for i in stems3 if not seqs12.linkedStems[i - cumTotNumStemsVec[1]]]
        #find corresponding stems earlier in fullSTableBPs
        correspondingDupStems = [i for j in duplicateStems for i in range(cumTotNumStemsVec[1]) \
                                  if fullSTableBPs[i] == fullSTableBPs[j]]
        
        # Check that duplicate stems and correspondingDupStems really are identical:
#        print([fullSTableBPs[i] for i in duplicateStems])
#        print([fullSTableBPs[i] for i in correspondingDupStems])

        fullSTableBPs = [fullSTableBPs[i] for i in range(len(fullSTableBPs)) if i not in duplicateStems]
        cumTotNumStemsVec[2] -= len(duplicateStems)
                
    #    now correct fullPossiblePermutationsList. First, change those
    #    stems in duplicateRegions to the corresponding earlier fullSTableBPs stems
        fullStructuresList = [[i if i not in duplicateStems else correspondingDupStems[duplicateStems.index(i)] \
                                for i in j] for j in fullStructuresList]
    
    #    and now change all stems >= duplicateRegions to what they
    #    are now that we've deleted duplicateRegions
        fullStructuresList = [sorted([i - sum([i > d for d in duplicateStems]) for i in j]) for j in fullStructuresList]
        #this maintains that each vector in structures is sorted for consistency 
        #(necessary for comparing vectors when we remove a stem)
        
        
        allComponentGraphs = seq1.allComponentGraphs + seq2.allComponentGraphs + seqs12.allComponentGraphs
        allNumVs = list(seq1.allNumVs) + list(seq2.allNumVs) + list(seqs12.allNumVs)
        bondFECounts = seq1.bondFECounts + seq2.bondFECounts + seqs12.bondFECounts
        dangling5Count = seq1.dangling5Count + seq2.dangling5Count + seqs12.dangling5Count
        dangling3Count = seq1.dangling3Count + seq2.dangling3Count + seqs12.dangling3Count
        unboundButCouldBindCounts = list(seq1.unboundButCouldBindCounts) + list(seq2.unboundButCouldBindCounts) + list(seqs12.unboundButCouldBindCounts)
                
        self.checkFxnStartTime = 0
        self.STARTtime = seq1.STARTtime + seq2.STARTtime + seqs12.STARTtime
        self.PERMUtime = seq1.PERMUtime + seq2.PERMUtime + seqs12.PERMUtime
        self.CHECKtime = seq1.CHECKtime + seq2.CHECKtime + seqs12.CHECKtime
        
        return(cumTotNumStemsVec, cumTotNumStructuresVec, fullSTableBPs, fullStructuresList,
               allComponentGraphs, allNumVs, bondFECounts, dangling5Count, dangling3Count, unboundButCouldBindCounts)
...
#                may in reality be pseudoknots of much lower order
    ...
    def makeFourwayCompatibilityTensor(self, C3):
# =============================================================================
...
# =============================================================================
        ...
        STableBPs = self.STableBPs
        CMat = self.C
#        C3 = self.C3 #We no longer store C3 or C4 because they're huge
                                    ...
                                    if C3[i,j,q] and C3[i,k,q] and C3[j,k,q] and not linkedStems[q]: 
                                        ...
                                        if list(np.argsort([A,a,B,b,C,c,D,d])) not in fourwayDisallowedPermutations:
                                            ...
                                            if counter % 5e5 == 0 and printProgressUpdate:
                                                myPrintFxn('Setting up C4 (fourway compatibility tensor): i = ' + str(i) + 
                                                           '; length c4 indices = ' + str(counter), self.fileTxt)
...
# =============================================================================
        postCalculationFxnStartTime = time.time()
...
# =============================================================================
        ...
        allLoopEntropiesNoVs = np.zeros(numStructures)
        ...
        for whichStruct in range(numStructures):
            ...
            allLoopEntropiesNoVs[whichStruct] = kB*loopEntropy
...
#        We cut this in half by only considering RNA(DNA) for systems that only have RNA(DNA)
        ...
        self.allLoopEntropiesNoVs = allLoopEntropiesNoVs
...
#        and its FE will be given by sortedFEs[n].
        #In other words, allFreeEnergies[indexSort] is idential to sortedFEs
        ...
        self.postCalculationFxnTime = time.time() - postCalculationFxnStartTime
        if printProgressUpdate:
            myPrintFxn('Time for postCalculationFxn function = ' + str(self.postCalculationFxnTime), self.fileTxt)
        
        self.completed = True
        
        if self.toSave: #saving can take some time
            if self.numStructures > 1e6:
                self.saveSmall = True
            ...
            if self.saveSmall:
                ...
                myPrintFxn('Time to save = ' + str(time.time() - timeToSave), self.fileTxt)
        ...
        for whichStruct in range(numStructures):
            ...
            (bondFECounts[whichStruct], dangling5Count[whichStruct], dangling3Count[whichStruct],
             specialLoopEnergies, specialLoopEntropies, unboundButCouldBindCounts[whichStruct]) = \
                 ...
                 #It's important to put the bond energy calculation last since
                 #it modifies the structure variable (specifically by removing stems of length 1)
                 #Do change this, have the input be copy.deepcopy(structure) instead
...
#             but we only include them in the FE calculation if includeTerminalAUATPenalties==True.
        ...
        for stem in structure: #two potential terminal mismatches for each stem
            ...
            for (firstNtIndex, firstBPIndex) in [[stem[numBonds], stem[0]], [stem[numBonds - 1], stem[-1]]]:
...
# =============================================================================
                if (np.sign(firstNtIndex - numNt1_1) == np.sign(secondNtIndex - numNt1_1) and
                    np.sign(firstBPIndex - numNt1_1) == np.sign(secondBPIndex - numNt1_1)):
                    #Don't consider stacking or dangling if the first bp is between a different
                    #set of strands than the second bp
                    bondsAcrossDifferentStrands = False
                else:
                    bondsAcrossDifferentStrands = True
                ...
                if ((secondNtIndex not in boundNts and secondBPIndex not in boundNts
                    ...
                    if not bondsAcrossDifferentStrands:
                        index, bpType = freeEnergyMatrixIndices(sequenceInNumbers, firstNtIndex,firstBPIndex,
...
#                        (i.e. we don't consider if secondBPIndex in boundNts)
                    ...
                    if (np.sign(firstNtIndex - numNt1_1) == np.sign(secondNtIndex - numNt1_1) and
                        np.sign(firstBPIndex - numNt1_1) == np.sign(secondBPIndex - numNt1_1)):
                        # Find bondsAcrossDifferentStrands for the new secondBPIndex
                        bondsAcrossDifferentStrands = False
                    else:
                        bondsAcrossDifferentStrands = True
                    if not bondsAcrossDifferentStrands:
#                        print([firstNtIndex, firstBPIndex, secondNtIndex, secondBPIndex])
#                        print([sequenceInNumbers[i] for i in [firstNtIndex, firstBPIndex, secondNtIndex, secondBPIndex]])
                        index, bpType = freeEnergyMatrixIndices(sequenceInNumbers, firstNtIndex,firstBPIndex,
                                                                secondNtIndex,secondBPIndex, bound = bound,
                                                                unmatchedBPPenalty = unmatchedBPPenalty)
                        bondFECounts[bpType][index[0], index[1]] += 1
...
# =============================================================================
                ...
                if firstNtIndex in lastNts and not bondsAcrossDifferentStrands: 
                        #Don't need to worry if lastNt is bound to firstNt
#                        since that'll be taken care of by danglingMatrixIndices
                    index, bpType = danglingMatrixIndices(
                            sequenceInNumbers, firstBPIndex, secondBPIndex, firstNtIndex)
                    dangling5Count[bpType][index[0], index[1]] += 1
                if firstBPIndex in firstNts and not bondsAcrossDifferentStrands: 
                    index, bpType = danglingMatrixIndices(
                            sequenceInNumbers, firstNtIndex, secondNtIndex, firstBPIndex)
    ...
    def calculateLoopEntropy(self,structure):
        G, numVs, bridgeList = createGraphFromStructure(structure, self.numNt, self.linkerPos, fileTxt = self.fileTxt)
        componentGraphs = calculateEntropyFromGraph(copy.deepcopy(G), self.refNets, bridgeList, fileTxt = self.fileTxt)
            #G is inputted as a copy since the function modifies it and we want 
            #to return the original (not decomposed) G
        return(numVs, componentGraphs, G)
        
        
    def enumerateStructures(self, C3=True, C4=True):
# =============================================================================
#        Make the list of structures. Each structure is defined by the list of stems that comprise it
#        (previously helipoints)
# =============================================================================
        numStems = self.numStems
        C = self.C; #C3 = self.C3; C4 = self.C4 #We no longer store C3 & C4 in self because they're huge
        printProgressUpdate = self.printProgressUpdate
...
#                                    Iterate over all triplets in current structure
                        ...
                        if frozenStems:
                            for stemList in frozenStems:
                                if set(stemList).isdisjoint(currStructure):
                                    hasEnoughBondedStrands = False  # doesn't have a frozen stem it needs to have
                        ...
                        if minNumStemsInStructure <= lenCurrStructure and hasEnoughBondedStrands:
                            structures.append(copy.copy(currStructure)) #even better, make it numpy array.
                                #copy.copy() makes it invariant to pop() and append() modifications
                            numStructures += 1
        ...
        if minNumStemsInStructure <= 1:
            for i in range(numStems): #each stem can be its own helipoint
                if (linkedStems[i] or not onlyConsiderBondedStrands) and not frozenStems:
                    structures.append([i])
                    numStructures += 1
                elif frozenStems:
                    compatibleWithFrozen = True
                    for stemList in frozenStems:
                        if set(stemList).isdisjoint([i]):
                            compatibleWithFrozen = False  # doesn't have a frozen stem it needs to have
                    if compatibleWithFrozen:
                        structures.append([i])
                        numStructures += 1
 
            
        if minNumStemsInStructure == 0 and not frozenStems: #then a completely free structure is allowed as well
            structures.append([])
            numStructures += 1
...
#        come up with an example off the top of my head
        ...
        correctedFrozenStems = copy.copy(frozenStems) 
            #doesn't really have to be a copy since we're going to overwrite frozenStems 
            #with correctedFrozenStems when the function returns
...
# =============================================================================
        ...
        allowPseudoknotsTrue = copy.copy(allowPseudoknots)
                ...
                if i == j:
                    ...
                    if numSequences > 1:
                        if linkedStems[i] or linkedStems[j]:
                            ...
                            allowPseudoknots = allowPseudoknotsTrue
                    if not allowPseudoknots:
# =============================================================================
...
# =============================================================================
                        ...
                        b = STableStructure[i][len(STableStructure[i])//2]
                        c = STableStructure[j][0]
                        d = STableStructure[j][len(STableStructure[j])//2]
                        
                        if c < a: #switch around labels so we have c>a
                            a = STableStructure[j][0]
                            b = STableStructure[j][len(STableStructure[j])//2]
                            c = STableStructure[i][0]
                            d = STableStructure[i][len(STableStructure[i])//2]
                    ...
                    #Right now, this only constrains pairwise compatible stems, but it's a start
                    iHairpin = list(range(STableStructure[i][len(STableStructure[i])//2 - 1] + 1, 
                                         STableStructure[i][len(STableStructure[i])//2]))
                    #The unpaired nts between the start and end of stem i
                    jHairpin = list(range(STableStructure[j][len(STableStructure[j])//2 - 1] + 1, 
                                         STableStructure[j][len(STableStructure[j])//2]))
                    #same for stem j
            ...
            for i in range(numStems): #for each stem
                if C[i,i]: #not important here, but useful when we repeat this process after the next block.
                    for j in range(len(frozenStems)): #for each set of stems that need to be included
                        ...
                        if not compatibleWithFrozen: #if for any set of stems one of which needs to be included,
                            #stem i isn't compatible with any of them, we can't include it in our structure
                            C[i,:] = 0
                            C[:,i] = 0
...
# =============================================================================
        for i in range(numStems):
            for j in range(i+1,numStems):
                if C[i,j]: #not worth going through this code if we already know the stems aren't compatible
...
# =============================================================================
    ...
    def frozenStemsFromFrozenBPs(self):
# =============================================================================
#        given a list of base pairs that we want fixed, what stems do you need to have to include all of them?
...
#        As expected, this doesn't give good results (but doesn't return an error)
#        if you try to force two incompatible base pairs (for example two base pairs using the same ntd)
# =============================================================================
        ...
        for j in range(len(frozenStems)):
            if not frozenStems[j]: #if we couldn't find any stems in STable that'll satisfy the j'th stem in frozenStructBPs
...
#        if onlyConsiderBondedStrands: Should consider intramolecular stems regardless, since even if onlyConsiderBondedStrands,
...
# =============================================================================
    ...
    def returnFxn(self):
        ...
        if self.printProgressUpdate:
            myPrintFxn('Total time = ' + str(time.time() - self.timeAtStart), self.fileTxt)
            numStructuresToPrint = min(self.numStructures, 5)
            myPrintFxn('sortedProbs = ' + str(self.sortedProbs[:numStructuresToPrint]), self.fileTxt)
            myPrintFxn('sortedFEs = ' + str(self.sortedFEs[:numStructuresToPrint]), self.fileTxt)
            # myPrintFxn('indexSort = ' + str(self.indexSort[:numStructuresToPrint]), self.fileTxt)
            
            mfeStructures = self.MFEStructures(numStructuresToPrint)
            myPrintFxn('Top MFE structures = ', self.fileTxt)
            for i in range(numStructuresToPrint):
                myPrintFxn(str(mfeStructures[i]), self.fileTxt)
                
        return(self)
        
    
    def saveSmallFxn(self):
        # If there are too many structures, don't save allowing for changing parameters
        # just save assuming the parameters are what we set
        
        x = LandscapeFold(
                self.sequences, 
                DNA=self.DNA, 
                sequenceConcentrations=self.sequenceConcentrations, 
                b=self.b, 
                T=self.T, 
                vs=self.vs, 
                duplexEntropyPenaltyInKB=self.duplexEntropyPenaltyInKB, 
                ...
                toSave=self.toSave,
                saveSmall = True,
                tryToLoadVariables=self.tryToLoadVariables, 
                maxSizeSTable=self.maxSizeSTable, 
                onlyAllowSubsetsOfLongestStems=self.onlyAllowSubsetsOfLongestStems, 
                onlyConsiderSubstemsFromEdges=self.onlyConsiderSubstemsFromEdges,
                onlyConsiderBondedStrands=self.onlyConsiderBondedStrands, 
                considerTabulatedLoops=self.considerTabulatedLoops, 
        ...
        x.allBondEnergies = self.allBondEnergies
        x.allBondEntropies = self.allBondEntropies
        x.allDuplexEntropies = self.allDuplexEntropies
        x.allLoopEntropies = self.allLoopEntropies
        x.allNumVs = self.allNumVs
        x.fileFigs = self.fileFigs
        x.fileTxt = self.fileTxt
        x.fileVars = self.fileVars
        x.folderName = self.folderName
        x.frozenStems = self.frozenStems
        x.g = self.g
        x.indexSort = self.indexSort
        x.linkerPos = self.linkerPos
        x.linkedStems = self.linkedStems
        x.linkedStructures = self.linkedStructures
        x.numSequences = self.numSequences
        x.numStems = self.numStems
        x.numStructures = self.numStructures
        x.sequence = self.sequence
        x.sortedFEs = self.sortedFEs
        x.sortedProbs = self.sortedProbs
        x.STableBPs = self.STableBPs
        x.STableStructure = self.STableStructure
        x.structures = self.structures
        x.version = self.version
        
        timeToSave = time.time()        
        save(x, self.fileVars + '_small')
        myPrintFxn('Time to save small = ' + str(time.time() - timeToSave), self.fileTxt)
    ...
    def figureFxn(self):
        ...
        f2 = drawRNAStructureSeqCircle(self.structureWithNthLowestFE(0), self.sequence, 
                             linkerPos = self.linkerPos, showFig = True)
        if self.toSave:
            f2.savefig(fileFigs + 'MFE_RNA_Structure_circle.png')
        
        
        #Histogram of FEs
        f3 = self.histFEs()
        if self.toSave and f3:  # f3 may be False if we don't have enough free energies to histogram
            f3.savefig(fileFigs + 'FE_Histogram') # bbox_inches='tight' should get rid of extra whitespace
...
# =============================================================================
        trueTimeAtStart = self.timeAtStart
        trueMakeFigures = self.makeFigures
        if self.saveSmall:
            smallList = ['_small']  # first try to load small
        else:
            smallList = ['', '_small']
        for small in smallList:
            # First try to load whole thing; otherwise try to load small
            try: 
                loadedVars = load(self.fileVars + small)
                loadedVars.timeAtStart = trueTimeAtStart
                loadedVars.makeFigures = trueMakeFigures
    ...
    #          The simplest case is if we stored graphs in the past but don't need to now. 
                if not self.storeGraphs: 
                    ...
                    try:
                        loadedVars = load(fileVars + small)
    ...
    #               includeTerminalAUATPenalties, and/or includeDanglingEnds.
                ...
                truePrintProgressUpdate = self.printProgressUpdate
                trueToSave = self.toSave
                trueG = self.g
                trueB = self.b
                ...
                tryFiles = folderName + self.fileNameFxn(self.numSequences,str(self.pairwiseEnergies), 
                    ...
                    strUnboundButCouldBindPenaltiesE = '*', 
                    strUnboundButCouldBindPenaltiesS = '*') + 'vars' + small + '.*'
        ...
        #                    on the previous run, you won't be able to get the graphProbs.
                            ...
                            loadedVars.printProgressUpdate = truePrintProgressUpdate
                            loadedVars.toSave = trueToSave
                            loadedVars.g = trueG
                            loadedVars.b = trueB
...
#         #File is saved in the folder LandscapeFold_#version/numSeq_2/numNt_6/A/AB/ABC/ABCD/ABCDE/ABCDEF/numNt_4/G/GH/GHI/GHIJ/
# =============================================================================
                
        folderName = 'LandscapeFold_' + self.version + '/numSeq_' + str(self.numSequences) + '/'
        ...
        maxNumCharsToUse = 600 # even with maxNumCharsInFolderName, too large folder names can't get created
        # so I arbitrarily choose that it only does the first 600 characters of the sequence...

        
        for k in range(self.numSequences):
            numNtK = numNtVec[k]
            
            # If we have O's, this is a nicer way to deal with them.
            # Basically, our folder names are separated by the O's, and n O's in a row
            # get given a folder name of On
            seqSplitByO = self.sequences[k].split('O')
            seqSplitByOCompressed = []
            i = 0
            while i < len(seqSplitByO):
                if seqSplitByO[i]:
                    seqSplitByOCompressed.append(seqSplitByO[i])
                numOs = 1
                while i + 1 < len(seqSplitByO):
                    if seqSplitByO[i+1]:
                        break
                    else:
                        i += 1
                        numOs += 1
                if i < len(seqSplitByO) - 1:
                    seqSplitByOCompressed.append('O' + str(numOs))
                i += 1
            
            nNtNested = min(numNtK + 1,maxNumNtsNested)
            folderName += 'numNt_' + str(numNtK) + '/'
            folderName += 'DNA_' + str(self.DNA[k]) + '/'
            
            for i in range(1,nNtNested):  # Make the sequence start with e.g. G/GG/GGU/GGGUA/...
                folderName += self.sequences[k][:i] + '/'
                
            if numNtK >= maxNumNtsNested:
                for seqPieces in seqSplitByOCompressed:
                    numCharsUsed = 0
                    for i in range(len(seqPieces) // maxNumCharsInFolderName + 1):  
                        folderName += seqPieces[i*maxNumCharsInFolderName:(i+1)*maxNumCharsInFolderName] + '/'
                        numCharsUsed += len(seqPieces[i*maxNumCharsInFolderName:(i+1)*maxNumCharsInFolderName])
                        if numCharsUsed > maxNumCharsToUse:
                            break
...
# =============================================================================
    ...
    def profileFxn(self):
        cProfile.runctx('self.mainLandscapeCalculation()', globals(), locals())
...
#        its FE is given by self.sortedFEs[n]
        ...
        structure = [self.STableStructure[j] for j in self.structures[whichStruct]]
        return(structure)
        
        
    def MFEStructures(self, n):  # return the top n MFE structures
        n = min(n, self.numStructures)
        return([self.structureWithNthLowestFE(m) for m in range(n)])
        
        
    def nthEnumeratedStructure(self,n): #create the n'th structure given the structures and STable matrices
        return([self.STableStructure[j] for j in self.structures[n]])
...
#        Given a structure, find its index and position in self.sortedFEs
    ...
    def histFEs(self):
        # Histogram the free energies of all structures
        displayedFE = [self.sortedFEs[i] for i in range(self.numStructures) if self.sortedFEs[i] < 30]
        # Set an upper limit of 30 to exclude structures that aren't physical.
        
        if len(displayedFE) > 20:  # don't histogram if we don't have enough free energies to histogram
            if len(displayedFE) > 1000:
                nBins = min(500, int(len(displayedFE) / 50))
            else:
                nBins = int(len(displayedFE)/10)
            f = plt.figure()
            n, bins, patches = plt.hist(displayedFE, bins=nBins)
...
#             terminal_AT_penalty_energy, terminal_AT_penalty_entropy) = terminalAUATPenalties()
#        else:
#            (terminal_AU_penalty_energy, terminal_AU_penalty_entropy, 
...
def hammingDistIsOne_Sparse(X):
    #From StackExchange (using broadcasting to compute Hamming distance)
#    This is very slow for large BPmat
    #Perhaps this is faster? [(i,j) for i in range(len(BPmat)) for j in range(len(BPmat)) if (BPmat[i] == BPmat[j]).sum() == len(BPmat[0])-1]
    return(scipy.sparse.dok_matrix((X[:, None, :] != X).sum(2) == 1))
...
def terminalAUATPenalties():
    terminal_AT_penalty_energy = 2.2; terminal_AT_penalty_entropy = (2.2-0.05)/310.15
    #from SantaLucia and Hicks review The Termodynamics of DNA Structural Motifs 
...
def danglingMatrixIndices(sequenceInNumbers, pairedNtIndex, danglingNtIndex, pairedNtCompIndex):
# =============================================================================
#     Get the indices of the energy/entropy matrix to use for a dangling end#    
#    where pairedNt is the bonded nt on the same strand as the dangling end, 
#    and danglingNt is the dangling end
#    pairedNtCompIndex is the bonded nt on the opposite strand as the dangling end
#    Considering it allows us to treat GU pairs as if they were AU
# =============================================================================
    ...
    if danglingNtIndex < 0 or danglingNtIndex > numNt - 1: #pairedNt must be all right
        ...
        pairedNtComp = sequenceInNumbers[pairedNtCompIndex]
        
        if pairedNtComp == 4 and pairedNt == 3:  # treat GU pairs as if they were AU
            pairedNt = 1
...
def freeEnergyMatrixIndices(sequenceInNumbers, firstNtIndex,firstBPIndex, secondNtIndex,
                            secondBPIndex, bound = [1,1], unmatchedBPPenalty = True, fileTxt = ''):
# =============================================================================
...
#    that we should treat them as an AC pair (where A replaces the purine and C the pyrimidine)
    if unmatchedBPPenalty and not bound[1]:
        if isComplementary(secondNt, secondBP):
            if secondNt == 1 or secondNt == 3: #if secondNt is a purine (RNA)
            ...
            elif secondNt == 2 or secondNt == 4:  #if secondNt is a pyrimidine (RNA)
                secondNt = 2 #the pyrimidine is replaced with C
                
                if secondBP <= 4:
                    secondBP = 1 # and the purine with A
                else:
                    secondBP = 5
                    
            elif secondNt == 5 or secondNt == 7: #if secondNt is a purine (DNA)
                secondNt = 5 #the purine is replaced with A 
            ...
            else: #if secondNt is a pyrimidine (DNA)
                secondNt = 6 #the pyrimidine is replaced with C
...
# =============================================================================
    ...
    if bpType == 0: #we're dealing with an RNA/RNA bond
        ...
        if firstNt == 1 and firstBP == 4:
            ...
            myPrintFxn('The deltaG function has a problem RNA/RNA', fileTxt)
...
#        not being returned as separate indices, but as one index (4*secondNt + secondBP)
        ...
        if firstNt == 5 and firstBP == 8:
            ...
            myPrintFxn('The deltaG function has a problem DNA/DNA', fileTxt)
...
#        upside down a minute ago)
        ...
        if firstNt == 1 and firstBP == 8:
            ...
            myPrintFxn('The deltaG function has a problem RNA/DNA', fileTxt)
...
# =============================================================================
#     Return a list of the nts that are bound where the total length of the sequence is numNt
# =============================================================================
...
def bpsList2structMatrix(bpsList, numNt):
# =============================================================================
#     Return a matrix M where Mij = 1 if ntds i & j are paired and 0 otherwise
# =============================================================================
    M = np.zeros((numNt,numNt),dtype = bool)
    for i,j in bpsList:
        M[i,j] = True
        M[j,i] = True
    return(M)
    
def structure2structMatrix(structure, numNt):
    return(bpsList2structMatrix(structure2bpsList(structure),numNt))
    
def structBPs2structMatrix(structBPs, numNt):
    return(bpsList2structMatrix(structBPs2bpsList(structBPs),numNt))


def dotBracket2BPsList(dotBracket):
# =============================================================================
#     Takes as input a string defining a structure in dot-bracket notation
#     e.g. '(((...)))' and converts it to a list of base pairs, e.g. 
#     [[0, 8], [1, 7], [2, 6]]
# =============================================================================
    bpsList = []
    dotBracket = [i for i in dotBracket]  # convert string to list to support item assignment
    allDots = ['.' for i in range(len(dotBracket))]
    while dotBracket != allDots:
        for i in range(len(dotBracket) - 1)[::-1]:
            if dotBracket[i] == '(' and dotBracket[i+1] != '(':  # end of stem
                foundClose = False
                j = i
                while not foundClose:
                    j = j+1
                    if dotBracket[j] == ')':
                        foundClose = True
#                structBPs = [structBPs;[i,j]];
                bpsList.append([i, j])
                dotBracket[i] = '.'
                dotBracket[j] = '.'
                break
            
            if dotBracket[i] == '[' and dotBracket[i+1] != '[':
                foundClose = False
                j = i
                while not foundClose:
                    j += 1
...
def bpsList2dotBracket(bpsList, numNt):
# =============================================================================
#     Takes as input a list of base pairs converts it to dot-bracket notation
#     Maximum nesting of 3 stems (parentheses, brackets, braces)
# =============================================================================
    #tested on [[1, 13], [2, 12], [3, 11], [4, 16], [5, 15], [6, 14], [7, 20], [8, 19], [9, 18]]
    # and [[1, 10], [2, 9], [3, 8], [11, 20], [12, 19], [13, 18], [21, 30], [22, 29], [23, 28], [25, 37], [26, 36], [27, 35], [31, 40], [32, 39], [33, 38]]
    ...
    indexOpenBrace = []
    for i in range(numNt):
        if any([x[0]==i for x in bpsList]): #meaning that ntd i is the first nt of a bp
            j = [x for x in bpsList[np.where(
                    [i in x for x in bpsList]
                    )[0][0]] if x != i][0] #the ntd that i is bonded to (there must a better way to do this...)
            ijCanBeParen = True
            for k in range(len(indexOpenParen)):
                if ((i>indexOpenParen[k] and i<indexClosedParen[k]) and 
                    not (j>indexOpenParen[k] and j<indexClosedParen[k]) or
                    not (i>indexOpenParen[k] and i<indexClosedParen[k]) and 
                    (j>indexOpenParen[k] and j<indexClosedParen[k])):
                    ijCanBeParen = False
            ...
            if ijCanBeParen:
                ...
                ijCanBeBracket = True
                for k in range(len(indexOpenBracket)):
                    if ((i>indexOpenBracket[k] and i<indexClosedBracket[k]) and
                        not (j>indexOpenBracket[k] and j<indexClosedBracket[k]) or
                        not (i>indexOpenBracket[k] and i<indexClosedBracket[k]) and 
                        (j>indexOpenBracket[k] and j<indexClosedBracket[k])):
                        ijCanBeBracket = False
                if ijCanBeBracket:
                    dotBracket[i] = '['
                    dotBracket[j] = ']'
                    indexOpenBracket += [i]
                    indexClosedBracket += [j]
                else:
                    ijCanBeBrace = True;
                    for k in range(len(indexOpenBrace)):
                        if ((i>indexOpenBrace[k] and i<indexClosedBrace[k]) and
                            not (j>indexOpenBrace[k] and j<indexClosedBrace[k]) or
                            not (i>indexOpenBrace[k] and i<indexClosedBrace[k]) and 
                            (j>indexOpenBrace[k] and j<indexClosedBrace[k])):
                            ijCanBeBrace = False
                    if ijCanBeBrace:
                        dotBracket[i] = '{'
                        dotBracket[j] = '}'
                        ...
                        print('problem with structBPs2dotBracket2')
    return(''.join(dotBracket))


def dotBracket2dotBracketOrHairpin(dotBracket):
#    Given dot bracket notation, denote by '-' those free nts that are part of a hairpin loop
    
    dotBracket_orHairpin_list = np.array([i for i in dotBracket])
    potentialNtsInHairpin = []
    ntsInHairpin = []
    followingOpen = False
    for i in range(len(dotBracket)):
        if dotBracket[i] == '(':
            followingOpen = True
            potentialNtsInHairpin = []
        if followingOpen and dotBracket[i] == '.':
            potentialNtsInHairpin += [i]
        if dotBracket[i] == ')':
            ntsInHairpin += potentialNtsInHairpin
            potentialNtsInHairpin = []
            followingOpen = False
    if ntsInHairpin:
        dotBracket_orHairpin_list[np.array(ntsInHairpin)] = '-'
    dotBracket_orHairpin = ''.join(
            dotBracket_orHairpin_list)
    
    return(dotBracket_orHairpin)


def dotBracket2dotBracketOrLoop(dotBracket):
#    Given dot bracket notation, denote by '-' those free nts that are part of any loop
#    (hairpin, pseudoknot, internal, bulge,...)
    
    dotBracket_orFree_list = np.array([i for i in dotBracket])
    numOpenParens = 0
    numOpenBrackets = 0
    numOpenBraces = 0
    for i in range(len(dotBracket)):
        ...
        elif (dotBracket[i] == '.' and numOpenParens == 0 and 
            numOpenBrackets == 0 and numOpenBraces == 0):
            dotBracket_orFree_list[i] = '.'
        elif dotBracket[i] == '.':
            dotBracket_orFree_list[i] = '-'
            
    dotBracket_orFree = ''.join(
            dotBracket_orFree_list)
    
    return(dotBracket_orFree)



def structBPs2helipoints(structBPs, includeInterveningUnpairedNt=True):
    numStems = len(structBPs)
    
# =============================================================================
#    For each stem, make a list of the stems from which it is separated by at most one unpaired nt
#    Do this through creating a matrix adjStems where adjStems[i,j] = 1 iff the two stems
#    are separated by at most one unpaired nt. Set diagonal to one by convention. 
# =============================================================================
    adjStems = np.zeros((numStems,numStems),dtype = bool)
    
    if includeInterveningUnpairedNt:
        minDist = 2  # see comment below
    else:
        minDist = 1
    
    for i, stem in enumerate(structBPs):
        edgesOfI = [stem[0][0], stem[0][1], stem[-1][0], stem[-1][1]]  # the first bp, the last bp
        adjStems[i, i] = 1
        for jMinusIMinus1, stem2 in enumerate(structBPs[i+1:]):
            edgesOfJ = [stem2[0][0], stem2[0][1], stem2[-1][0], stem2[-1][1]]
            separationBetweenStems = [abs(x - y) for x in edgesOfI for y in edgesOfJ]
            #We're looking for there to be at least one 1 (meaning stem i is 
            #directly adjacent to j) or 2 (meaning there is an intervening
            #unpaired nt.) There can't be any 0's since then they wouldn't be mutually compatible.
            if any([x <= minDist for x in separationBetweenStems]):
                adjStems[i, jMinusIMinus1 + i + 1] = 1
                adjStems[jMinusIMinus1 + i + 1, i] = 1
    
    connComp = [i for i in nx.connected_components(nx.Graph(adjStems))]
    helipoints = [sorted(np.concatenate([structBPs[i] for i in j]).tolist()) for j in connComp]
    return(helipoints)


def helipoints2bpsList(helipoints):
    return([bp for helipoint in helipoints for bp in helipoint])
...
def myNumConnCompFromMatC(M, edges, fileTxt = ''): #same principle as above
# =============================================================================
...
#       to a power that's a multiple of 2, since the code raises to a power by successive self-multiplications.
    ...
    for i in range(nEdges):
        ...
        if any(np.sum(C,axis=0)) == 0:
            myPrintFxn('DIVIDE BY ZERO ERROR with M = ' + str(M) + '; edges = ' + str(edges), fileTxt)
...
def drawRNAStructure(structure,sequence,linkerPos = [], figsize = (7,7), showFig = True, 
                     backboneDist=1.2, hbondDist=1.3):
# =============================================================================
#    Makes a planar diagram of RNA structure. Doesn't work well for two sequences if they are not bound.
# =============================================================================
...
#    Make the backbone edges
    for i in range(numNt - 1):
        if i not in linkerPos and i+1 not in linkerPos:
            G.add_edge(nodelist[i],nodelist[i+1], backbone = True, weight=backboneDist) #higher weight means more distance between bonded nodes
    
#    Make the bond edges
    for stem in structure:
        numBonds = int(len(stem)/2)
        for i in range(numBonds):
            G.add_edge(nodelist[stem[i]], nodelist[stem[i+numBonds]], weight=hbondDist, backbone = False)
...
def drawRNAStructureSeq(structure,sequence,linkerPos = [], figsize = (7,7), showFig = True, 
                     backboneDist=1.2, hbondDist=1.3):
# =============================================================================
#    Same as drawRNAStructure but nodes are colored by sequence and no text is shown
# =============================================================================
    f = plt.figure(figsize=figsize)  
    numNt = len(sequence)
    G = nx.Graph()
...
#        We'll remove linker nodes later, but otherwise it makes nodelist confusing
        G.add_node(sequence[i] + str(i))
        if sequence[i] == 'A':
            nodeColors += [[230/256, 159/256, 0]]
        elif sequence[i] == 'C':
            nodeColors += [[86/256, 180/256, 233/256]]
        elif sequence[i] == 'G':
            nodeColors += [[0, 158/256, 115/256]]
        elif sequence[i] == 'U' or sequence[i] == 'T':
            nodeColors += [[213/256, 94/256, 0]]
        else: 
            nonStandardNt = True
            nodeColors += [[0.5, 0.5, 0.5]]
...
#    Make the bond edges
    ...
    pos = nx.kamada_kawai_layout(G,weight = 'weight')
    nx.draw_networkx(G, pos, edge_color=edgeColors, node_color = nodeColors, 
                     alpha = 0.7, with_labels=False)
    plt.axis('off')
    if showFig:
        plt.show()
        plt.figure()
        plt.plot([1,2], [1, 2], '.', color=[230/256, 159/256, 0], alpha=0.7, label='A', markersize=24)
        plt.plot([1,2], [1, 2], '.',color=[86/256, 180/256, 233/256], label='C', markersize=24)
        plt.plot([1,2], [1, 2], '.',color=[0, 158/256, 115/256], alpha=0.7, label='G', markersize=24)
        if 'U' in sequence and 'T' not in sequence:
            plt.plot([1,2], [1, 2], '.',color=[213/256, 94/256, 0], alpha=0.7, label='U', markersize=24)
        elif 'T' in sequence and 'U' not in sequence:
            plt.plot([1,2], [1, 2], '.',color=[213/256, 94/256, 0], alpha=0.7, label='T', markersize=24)
        elif 'U' in sequence and 'T' in sequence:
            plt.plot([1,2], [1, 2], '.',color=[213/256, 94/256, 0], alpha=0.7, label='T/U', markersize=24)
        if nonStandardNt:
            plt.plot([1,2], [1, 2], '.',color=[0.5, 0.5, 0.5], alpha=0.7, label='O', markersize=24) 
        plt.xlim([1, 10])
        plt.ylim([1, 10])
        plt.legend()
        plt.show()
...
def drawRNAStructureSeqCircle(structure,sequence,linkerPos = [], figsize = (7,7), 
                              showFig = True, returnFig=True):
# =============================================================================
#  Draws RNA structure as a circle
# =============================================================================
    endOfSeq = 'EE'  # The end of the sequence
    sequence += endOfSeq
    f = plt.figure(figsize=figsize)  
    numNt = len(sequence)
    G = nx.Graph()
...
#    Make the backbone edges
    for i in range(numNt - 1):
        if i not in linkerPos and i+1 not in linkerPos:
            G.add_edge(nodelist[i],nodelist[i+1], backbone = True, weight=0) #higher weight means more distance between bonded nodes
...
#    Make the bond edges
    ...
    for edge in G.edges:
        if int(edge[1][1:]) >= len(sequence) - len(endOfSeq):
            edgeColors.append('white')
        elif int(edge[1][1:]) - int(edge[0][1:]) == 1:  
            edgeColors.append('black')  # color of backbone edges
        else:
            edgeColors.append('black')  # color of H-bond edges
        
    pos = nx.kamada_kawai_layout(G,weight = 'weight')
    nx.draw_networkx(G, pos, edge_color=edgeColors, node_color=nodeColors, node_size=100,
...
def createGraphFromStructure(structure, numNt, linkerPos, fileTxt = ''): 
...
# =============================================================================
    ...
    #                is connected to the first nt (which hasn't yet been made into a node) 
                    ...
                    if len(bondedNtNodeDict[connectingNt]) == 1: #there's only one entry for this node
                        ...
                        myPrintFxn('Weird! We weren''t supposed to make the edge between nts ' + str(bondedNt) + ', ' + str(connectingNt), fileTxt)
                    ...
                    if bondedNt == 0: 
                        ...
                        if bondedNt == linkerPos[-1] + 1: #then it's the first nt of the second sequence
                            dictEntry[1] = False
                    if dictEntry[1]:
                        myPrintFxn('Could not find the previous bonded nt for nt ' + str(bondedNt), fileTxt)
    ...
    #                is connected to the last nt (if we don't make it into a node) 
                    ...
                    if len(bondedNtNodeDict[connectingNt]) == 1: #there's only one entry for this node
                        ...
                        myPrintFxn('Weird (2)! We weren''t supposed to make the edge between nts ' + str(bondedNt) + ', ' + str(connectingNt), fileTxt)
                    ...
                    if bondedNt == numNt - 1: 
                        ...
                        if bondedNt == linkerPos[0] - 1: #then it's the last nt of the first sequence
                            dictEntry[2] = False
                    if dictEntry[2]:
                        myPrintFxn('Could not find the next bonded nt for nt ' + str(bondedNt), fileTxt)
    ...
    for bondedNt in bondedNtNodeDict.keys():
        for dictEntry in bondedNtNodeDict[bondedNt]:
            if dictEntry[1]:
                myPrintFxn('Never found the preceding nt to make single edge for nt ' + str(bondedNt), fileTxt)
            if dictEntry[2]:
                myPrintFxn('Never found the next nt to make single edge for nt ' + str(bondedNt), fileTxt)
...
# =============================================================================
    ...
    #this also doesn't remove edges of a node connected to itself since M[i,i] == 3 if the edge is present in G.edges
    whichEdgesToRemove = myNumConnCompFromMatC(M[:nodeCounter,:nodeCounter].astype(bool), edgeList, fileTxt=fileTxt)
...
#    to itself comes in the middle of a single-bond stem of a larger structure (say a pseudoknot), it's tricky
...
def calculateEntropyFromGraph(G, refNets, bridgeList, fileTxt = ''):
# =============================================================================
...
#                in which case which key was zero and which was one would be arbitrary
            ...
            myPrintFxn('couldn''t match net', fileTxt)
            myPrintFxn('numBonds adjacency matrix = ', fileTxt)
            myPrintFxn(nx.to_numpy_matrix(Gsub), fileTxt)
            myPrintFxn('singleBonds adjacency matrix = ', fileTxt)
            myPrintFxn(nx.to_numpy_matrix(Gsub, weight='singleBond'), fileTxt)
...
# =============================================================================
# Entropy calculation given graph (formale found in Supplementary Fig. S2 of Kimchi et al. 2019), and limits thereof
# =============================================================================    
...
#     We also want to use this also just to print statements even outside of 
#     the main class in which case the fileTxt may be ''
# =============================================================================
    if fileTxt: 
